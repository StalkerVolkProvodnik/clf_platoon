...
// UPP Airborne
	...
	if(istype(squad, /datum/squad/marine/upp/airborne))
		var/squad_clr = H.assigned_squad.equipment_color
		switch(GET_DEFAULT_ROLE(_role))
			if(JOB_SQUAD_MEDIC) hud_icon_state = "med"
			if(JOB_SQUAD_SMARTGUN) hud_icon_state = "gun"
...
// UPP Forecon
	...
	if(istype(squad, /datum/squad/marine/upp/forecon))
		var/squad_clr = H.assigned_squad.equipment_color
		switch(GET_DEFAULT_ROLE(_role))
			if(JOB_SQUAD_MEDIC_FORECON_UPP) hud_icon_state = "med"
			if(JOB_SQUAD_SMARTGUN_FORECON_UPP) hud_icon_state = "gun"
			if(JOB_SQUAD_RTO_FORECON_UPP) hud_icon_state = "rto"
			if(JOB_SQUAD_TEAM_LEADER_FORECON_UPP) hud_icon_state = "tl"
			if(JOB_SQUAD_LEADER_FORECON_UPP) hud_icon_state = "leader_a"
		if(squad.fireteam_leaders["SQ1"] == H || squad.fireteam_leaders["SQ2"] == H)
			H.langchat_styles = "langchat_smaller_bolded"
		else
			H.langchat_styles = initial(H.langchat_styles)
...
/datum/faction/upp/get_antag_guns_snowflake_equipment()
	return list(
		...
		list("AG80 Submachinegun", 20, /obj/item/weapon/gun/rifle/ag80, null, VENDOR_ITEM_REGULAR),
		list("Type 71 Pulse Rifle", 30, /obj/item/weapon/gun/rifle/type71, null, VENDOR_ITEM_REGULAR),

		list("PRIMARY AMMUNITION", 0, null, null, null),
		list("AG80 Magazine (7.62x19mm)", 5, /obj/item/ammo_magazine/rifle/ag80, null, VENDOR_ITEM_REGULAR),
